...
const LOCAL_CHAT_URL: &str = "http://localhost:3000";
...
///   2. `http://localhost:3000` if the local Next.js controller answers a
///      TCP connect within 250ms
///   3. `https://closedmesh.com` (the hosted UI talks back to the local
///      controller via CORS — see root README "Public deployment")
pub fn preferred_url() -> String {
    ...
    if local_controller_up() {
        return LOCAL_CHAT_URL.to_string();
    }
...
/// "Show Logs" from the tray. The runtime + installer write logs to
/// platform-specific dirs (matches `public/install.sh`).
pub fn log_dir() -> Option<PathBuf> {
    let home = dirs::home_dir()?;
    let candidate = if cfg!(target_os = "macos") {
        home.join("Library/Logs/closedmesh")
    } else if cfg!(target_os = "linux") {
        home.join(".local/state/closedmesh")
    } else {
        // On Windows the Scheduled Task installer writes to %APPDATA%\closedmesh\logs
        dirs::data_dir()?.join("closedmesh").join("logs")
    };
    if candidate.exists() {
        Some(candidate)
    } else {
