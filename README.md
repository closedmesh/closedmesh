...
Next.js bundle either:

- bundled inside the **ClosedMesh desktop app** (`.dmg` / `.msi` / etc.)
  and spawned automatically as a Tauri sidecar on launch (default since
  desktop-v0.1.2 — see [`desktop/SIDECAR.md`](desktop/SIDECAR.md)), or
- installed as a launchd / systemd-user service via the legacy
  [`scripts/install-controller.sh`](scripts/install-controller.sh) for
  headless / power-user setups.

Either way, the browser's cross-origin call works because:
...
- Visitors with no local mesh get a "No local mesh detected" empty state with
  a download link — no half-broken chat UI.
